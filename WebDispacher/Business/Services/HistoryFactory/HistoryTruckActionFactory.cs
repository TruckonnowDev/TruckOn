...
{
    public class HistoryTruckActionFactory : IHistoryActionFactory<HistoryTruckAction, TruckViewModel>
    {
        public void CompareAndSaveUpdatedFields(object original, object updated, List<HistoryTruckAction> historyEntries, int truckId, HistoryActionData actionData, DateTime dateTimeUpdate, string prefix = "")
        {
            ...
            foreach (var property in properties)
            {
                if (Attribute.IsDefined(property, typeof(HistoryAttribute)))
                {
                    ...
                    if (!Equals(originalValue, updatedValue) && !(string.IsNullOrEmpty(originalValue.ToString()) && updatedValue == null))
                    {
                        ...
                        historyEntries.Add(new HistoryTruckAction
                        {
                            TruckId = truckId,
                ...
                if (property.PropertyType.IsClass && !property.PropertyType.IsPrimitive && property.PropertyType != typeof(string))
                {
                    ...
                    {
                        CompareAndSaveUpdatedFields(originalNested, updatedNested, historyEntries, truckId, actionData, dateTimeUpdate, property.Name + " ");
                    }
                }
            }
        }
